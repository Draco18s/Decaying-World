...
public class RustFogDecay extends BlockSand
{
    public RustFogDecay(int par1)
    {
        super(par1, Material.air);
        setHardness(-1F);
        setUnlocalizedName("Corrosive Fog");
        setLightOpacity(1);
    ...
    {
        blockIcon = iconRegister.registerIcon("DecayingWorld:cfog");
    }
    ...
    public void onEntityCollidedWithBlock(World par1World, int par2, int par3, int par4, Entity par5Entity)
    {
    	if (!par1World.isRemote) {
	        if(par5Entity instanceof EntityPlayer) {
	        	...
	        	int timer = nbt.getInteger("RustTimer") + 1;
	        	if(timer > 180) {
	        		timer = par1World.rand.nextInt(30);
		        	ItemStack a = player.getCurrentArmor(0);
		        	ItemArmor ar;
		        	if(a != null) {
		        		ar = (ItemArmor)(a.getItem());
			        	if(ar.getArmorMaterial() != EnumArmorMaterial.GOLD)
			        		a.damageItem(1, player);
		        	}
		        	a = player.getCurrentArmor(1);
		        	if(a != null) {
		        		ar = (ItemArmor)(a.getItem());
			        	if(ar.getArmorMaterial() != EnumArmorMaterial.GOLD)
			        		a.damageItem(1, player);
		        	}
		        	a = player.getCurrentArmor(2);
		        	if(a != null) {
		        		ar = (ItemArmor)(a.getItem());
			        	if(ar.getArmorMaterial() != EnumArmorMaterial.GOLD)
			        		a.damageItem(1, player);
		        	}
		        	a = player.getCurrentArmor(3);
		        	if(a != null) {
		        		ar = (ItemArmor)(a.getItem());
			        	if(ar.getArmorMaterial() != EnumArmorMaterial.GOLD)
			        		a.damageItem(1, player);
			        }
		        	a = player.getCurrentEquippedItem();
		        	if(a != null) {
			        	Item ai = a.getItem();
			        	...
			        	else if(ai instanceof ItemSword) {
			        		ItemSword is = (ItemSword)(ai);
				        	if(is.getToolMaterialName() != EnumToolMaterial.GOLD.toString())
				        		a.damageItem(1, player);
			        	}
			        	else {
			        		a.damageItem(1, player);
	        	...
	        	nbt.setInteger("RustTimer", timer);
	        }
        }
    }
